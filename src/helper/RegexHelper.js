...
class RegexHelper {
     ...
     * 
     * ex) regexHelper.value('#user_id', '아이디 값을 입력하세요');
     */
    value(field, msg) {
    ...
    /**
     * 두 값이 동일한지 검사한다.
     * @param {string} origin     원본에 대한 <INPUT>요소의 DOM객체
     * @param {string} compare    검사 대상에 대한 <INPUT> 요소의 DOM객체
     * @param {string} msg        검사에 실패할 경우 표시할 메시지
     */
    compareTo(origin, compare, msg) {
        this.value(origin, msg);
        this.value(compare, msg);
        var src = origin.value.trim();    // 원본 값을 가져온다.
